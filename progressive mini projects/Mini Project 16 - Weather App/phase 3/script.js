...
async function getWeather(city){
	try{
		...
		const weatherURL=`https://api.open-meteo.com/v1/forecast?latitude=${latitude}&longitude=${longitude}&current=temperature_2m,apparent_temperature,relative_humidity_2m,precipitation,weather_code,wind_speed_10m&daily=weather_code,temperature_2m_max,temperature_2m_min&timezone=auto&forecast_days=5`;
...
function displayForecast(dailyWeather){
	...
	dailyWeather.time.forEach(function(day,index){
		...
		const weatherInfo=getWeatherCondition(dailyWeather.weather_code[index]);
		icon.textContent=weatherInfo.icon;
		const weatherText=document.createElement("p");
		weatherText.textContent=weatherInfo.text;
		const max=document.createElement("p");
		max.textContent=`Max: ${dailyWeather.temperature_2m_max[index]}°C`;
		const min=document.createElement("p");
		min.textContent=`Min: ${dailyWeather.temperature_2m_min[index]}°C`;
